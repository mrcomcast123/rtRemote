 ...
 */
const resolve = new RTRemoteMulticastResolver_1('224.10.10.12', 10004);
...
resolve.start()
  .then(() => resolve.locateObject('some_name'))
      ...
      Promise.resolve()
        ...
        .then(() => checkMethodNoReturn(
          ...
          RTValueHelper.create((rtValueList) => {
            logger_1.debug(`function invoke by remote, args count =  + ${rtValueList.length}`);
            rtValueList.forEach((rtValue) => {
              logger_1.debug(`value=${rtValue.value}, type=${helper.getTypeStringByType(rtValue.type)}`);
            });
            logger_1.debug('function invoke by remote done');
          }, RTValueType.FUNCTION),
          RTValueHelper.create(10, RTValueType.INT32)
        ))
        .then(() => {
          logger_1.debug(`========= ${succeed} of ${total} example succeed, ${total - succeed} failed.`);
          logger_1.debug('test completed, next test will at 10s ...');
...
function checkMethod(rtObject, methodName, expectedValue, ...args) {
  return rtObject.sendReturns(methodName, ...args).then((rtValue) => {
    ...
    succeed += result ? 1 : 0;
    logger_1[result ? 'debug' : 'error'](`test method ${methodName} result = [${result}]`);
  });
}
...
function checkMethodNoReturn(rtObject, methodName, ...args) {
  return rtObject.send(methodName, ...args).then(() => {
    logger_1.debug(`test method ${methodName} result = [true]`);
    total += 1;
    succeed += 1;
  });
}
...
var sampleServerTest = {

};

module.exports = sampleServerTest;
module.exports = { run };
