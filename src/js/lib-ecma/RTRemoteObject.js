...
class RTRemoteObject {
  ...
  constructor(protocol, objectId) {
    logger.debug("RTRemoteObject::constructor objectId="+objectId);
  ...
  dump(msg)
  {
    logger.debug("RTRemoteObject dump:"+this.id+" msg:"+msg);
  }
   ...
   */
  set(prop, value) {
    logger.debug("RTRemoteObject::set");
   ...
   */
  get(prop) {
    logger.debug("RTRemoteObject::get");
    if (typeof prop === 'number') {
   ...
   */
  send(name, ...args) {
    logger.debug("RTRemoteObject::send");
  ...
  sendReturns(name, ...args) {
    logger.debug("RTRemoteObject::sendReturns");
    return this.protocol.sendCallByName(this.id, name, ...args);
  }
}
